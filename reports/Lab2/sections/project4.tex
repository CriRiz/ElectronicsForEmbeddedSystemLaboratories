...
To ensure that it is not an error, the code reads the line state after half a bit period (see line 21 of \ref{lst:V1}). 
If the check passed, the FPGA computes the sampling times and then begins sampling the other 8 data bits of the character encoded in ASCII, as shown in the for loop on line 28.\\
The 38th line makes sure that the trasmission ends with a stop bit (equal to 1) and then it builds the character (for loop in line 42).\\
