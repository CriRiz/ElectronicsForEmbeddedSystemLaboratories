...
In the first scenario, the aim is to send a single character through the transmission data register, as it can be seen in line 30 of the code in Listing \ref{lst:P31}, and then compare the STATUS register before and after the transmission, as illustrated in lines 23–24 and 32–33 in the code in the Listing \ref{lst:P31}.
\begin{listing}[ht]
...
\begin{itemize}
    ...
    \item E.
\end{itemize}
It can be observed that the TRDY flag is no longer active after the character is written to the transmission data register in line 30 of the code (Listing \ref{lst:P31}).
...
In the last scenario, Scenario 2 is repeated, but three characters are transmitted instead of two (as shown in lines 24–26 of \ref{lst:P33}).
\begin{listing}[ht]
\begin{minted}[linenos]{c}
int main() {
  char msg = 'f';
